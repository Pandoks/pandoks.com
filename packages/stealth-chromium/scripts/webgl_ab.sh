#!/usr/bin/env bash
# Bisect why WebGL is disabled in the production launch path. Runs webgl_ab.py
# over each mode (one browser per process) on a clean EC2 box via the SFN.
#   command="bash packages/stealth-chromium/scripts/webgl_ab.sh"
set -uo pipefail
...
WORK=/tmp/webglab
mkdir -p "$WORK"
LOG="$WORK/webgl-ab.log"
exec > >(tee "$LOG") 2>&1
S3="s3://${BUILDER_ARTIFACTS_BUCKET}/${BUILD_ID}"
trap 'aws s3 cp "$LOG" "${S3}/webgl-ab.log" >/dev/null 2>&1 || true' EXIT
...
echo "  vulkan ICDs visible to loader:"; ls /usr/share/vulkan/icd.d/ 2>&1 || echo "  (none in /usr/share)"
cd "$REPO_ROOT/packages/stealth-browser"; uv sync

echo "=== [3/3] WebGL launch bisect (one browser/process) ==="
for i in 0 1 2 3 4 5; do
  out=""
  for try in 1 2 3; do
    pkill -x chrome 2>/dev/null; sleep 1
    out=$(timeout 70 xvfb-run -a -s "-screen 0 1920x1080x24" \
      uv run python "$PKG_ROOT/scripts/webgl_ab.py" "$i" 2>/dev/null | grep '^AB')
    [ -n "$out" ] && break
  done
  [ -n "$out" ] && echo "  ${out#AB }" || echo "  [SKIP] mode $i (launch failed x3)"
done

echo "=== done -> ${S3}/webgl-ab.log ==="
