...
EDITS = [
    ...
    # --- MAX_VIEWPORT_DIMS (live-queried via GetWebGLIntArrayParameter) -
    # Viewport dims track MAX_TEXTURE_SIZE on real GPUs; they're live-queried
    # (not cached), so spoof in the array helper before the DOMInt32Array is
    # built. Clamp both axes to the persona's MAX_TEXTURE_SIZE.
    {
        "file": "third_party/blink/renderer/modules/webgl/"
                "webgl_rendering_context_base.cc",
        "header": '#include "apex_fingerprint.h"',
        "marker": "apex-webgl-viewport",
        "anchor": "  return WebGLAny(script_state,\n"
                  "                  DOMInt32Array::Create("
                  "base::span(value).first(length)));\n",
        "where": "before",
        "inject": "  // apex-webgl-viewport: viewport dims track MAX_TEXTURE_SIZE\n"
                  "  if (pname == GL_MAX_VIEWPORT_DIMS &&\n"
                  "      apex_fp::HasOverride("
                  "\"APEX_FP_WEBGL_MAX_TEXTURE_SIZE\")) {\n"
                  "    GLint apex_vp = static_cast<GLint>(apex_fp::EnvU32(\n"
                  "        \"APEX_FP_WEBGL_MAX_TEXTURE_SIZE\", value[0]));\n"
                  "    value[0] = apex_vp;\n"
                  "    value[1] = apex_vp;\n"
                  "  }\n",
    },
