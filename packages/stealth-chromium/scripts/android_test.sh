...
WORK=/tmp/androidtest; mkdir -p "$WORK"
LOG="$WORK/android-test.log"; exec > >(tee "$LOG") 2>&1
S3="s3://${BUILDER_ARTIFACTS_BUCKET}/${BUILD_ID}"
trap 'aws s3 cp "$LOG" "${S3}/android-test.log" >/dev/null 2>&1 || true' EXIT
...
echo "=== Android CDP-emulation probe ==="
pkill -x chrome 2>/dev/null; sleep 1
timeout 90 xvfb-run -a -s "-screen 0 1920x1080x24" \
  uv run python "$PKG_ROOT/scripts/android_probe.py" 2>&1 \
  | grep -E 'ANDROID_RESULT|Error|Traceback' || echo "  (no result)"
echo "=== done ==="
