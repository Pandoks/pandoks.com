...
func MigrateSlot(ctx context.Context, slot int64, migrationRoute MigrationRoute, clients []valkey.Client, currentTopology *ClusterTopology) error {
	...
	for {
		...
		keysInSlotCmd := sourceClient.B().ClusterGetkeysinslot().Slot(slot).Count(100).Build()
		...
		migrateKeysCmd := sourceClient.B().
			Migrate().
			Host(destNode.Address.Host).
			Port(destNode.Address.Port).
			Key("").
			DestinationDb(0).
			Timeout(5000).
			Keys(keys...).
