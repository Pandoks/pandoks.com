...
func (r *ValkeyClusterReconciler) reconcileConfigMap(ctx context.Context, cluster *valkeyv1.ValkeyCluster) error {
	desired := &corev1.ConfigMap{
		...
		Data: map[string]string{
			valkeyConfigFileName: renderValkeyConfig(&cluster.Spec),
		},
	}
	...
	needsUpdate := !reflect.DeepEqual(existing.Data, desired.Data) || !reflect.DeepEqual(existing.Labels, desired.Labels)
...
func configMapName(cluster *valkeyv1.ValkeyCluster) string {
...
func renderValkeyConfig(spec *valkeyv1.ValkeyClusterSpec) string {
	builder := &strings.Builder{}

	builder.WriteString("# Generated by valkey-operator\n")
	builder.WriteString("bind 0.0.0.0\n")
	builder.WriteString("protected-mode no\n")
	builder.WriteString(fmt.Sprintf("port ${PORT}\n"))
	builder.WriteString("\n")
	builder.WriteString("cluster-enabled yes\n")
	builder.WriteString("cluster-config-file nodes.conf\n")
	builder.WriteString("cluster-node-timeout ${CLUSTER_NODE_TIMEOUT}\n")
	builder.WriteString("\n")

	if containsPersistence(spec.Persistence, valkeyv1.PersistenceAOF) {
		builder.WriteString("appendonly yes\n")
		builder.WriteString("appendfsync everysec\n")
	} else {
		builder.WriteString("appendonly no\n")
	}

	if containsPersistence(spec.Persistence, valkeyv1.PersistenceRDB) {
		builder.WriteString("save 900 1\n")
		builder.WriteString("save 300 10\n")
		builder.WriteString("save 60 10000\n")
	} else {
		builder.WriteString("save \"\"\n")
	}

	builder.WriteString("dir /data\n")
...
func containsPersistence(modes []valkeyv1.ValkeyPersistenceMode, target valkeyv1.ValkeyPersistenceMode) bool {
