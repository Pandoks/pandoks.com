...
| `install.sh`    | `install_mise`, `install_mise_tools`, `install_global_tools` (`[_.global_tools]` dev conveniences — **skipped when `CI` is set**: claude-code/codex install breakage upstream must never fail CI's `pnpm install` preinstall), `install_docker`, `install_system_tools` (openssl/htpasswd), `install_aws_config`, `reload_or_hint` (post-install hint / `--reload` exec), `stream_track`, `cmd_bootstrap_all` (`--reload` parse + concurrent fork/join + cloud env-file write) |
