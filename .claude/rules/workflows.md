...
CI retains the OVH credentials and project ID and runs the TypeScript topology
contracts through `pnpm test:infra`; it does not source topology from CI
variables. `OVH_UNPROTECTED_NODE_LOGICAL_NAME` remains a temporary operator-only
scale-down override. Do not configure it as a persistent CI variable; it cannot
unprotect a lower-index or multiple nodes.
