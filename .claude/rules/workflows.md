...
pnpm run cluster deploy {local|dev|prod} [--bootstrap] [--stage NAME]
                                          ...
                                          [--quiet|-q]
pnpm run cluster test {postgres|valkey|clickhouse|all} [--keep] [--parallel]
```

`cluster test` runs the per-package Go suites at
`packages/<pkg>/test/cluster_test.go` (each exposed as a `test:cluster`
package script = `go test -C ./test`), built on the shared `packages/testkit`
module, against the local k3d cluster. It needs the cluster + deps up and
images pushed (`pnpm docker:build && pnpm dev:push`) but NO SST/AWS
credentials — suites helm-install charts directly from `packages/<pkg>/chart`.
`--keep` leaves passing suites' releases/namespaces in place; failures always
leave them. `test all --parallel` runs the suites concurrently on the shared
cluster (namespace-isolated; output buffered per suite) — a local-dev
speedup, not used in CI.
...
| `cluster-tests.yaml`     | push to main, PR to main                                                                                                                                                    | paths-filter per database package (`packages/{postgres,valkey,clickhouse}/**`) plus a `harness` filter (`packages/testkit/**`, `scripts/cluster/**`, `scripts/lib/**`, `docker-compose.yaml`, the workflow itself) that selects all three; `detect-changes` emits the changed set as a JSON matrix. `cluster-tests` is a **matrix job per changed package** (`fail-fast: false`, `timeout-minutes: 30`) — each on its own runner with its own k3d cluster: setup-node-pnpm + setup-mise (`k3d kubectl helm`) + setup-go, `pnpm cluster k3d deps up && k3d up`, that package's `docker:build`/`dev:push`, `./scripts/cluster/main.sh test <pkg>`. Measured: matrix ≈11m wall vs 17m sequential. `cluster-tests-pass` aggregator for branch protection. |
