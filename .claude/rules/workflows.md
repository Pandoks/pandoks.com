...
The checks and infrastructure-deploy jobs read the four counts and four
dedicated catalog fields from non-secret GitHub environment variables, then run
`scripts/cluster/validate-topology-env.sh` before any SST command. Configure
them in both the `production` and `dev` GitHub environments. Do not configure
`OVH_UNPROTECTED_NODE_LOGICAL_NAME` as a persistent CI variable; targeted
unprotect remains an explicit operator action.
...
| File                     | Triggers                                                                                                                                                                    | Does                                                                                                                                                                                                                                                                                                                                                                                                                                                                              |
| ------------------------ | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `deploy-infra.yaml`      | push to main on `infra/**`, `apps/**` (excl. `desktop-template`/`example`), `packages/svelte/**`, `k3s/**`, `scripts/cluster/**`; manual dispatch (`stage`/`deploy` inputs) | Map and validate non-secret OVH topology vars (`:73-94`), install, AWS OIDC, `pnpm sst refresh` (`continue-on-error: true`, `:104-106`), `pnpm sst deploy`. On success: Tailscale + ArgoCD `refresh=hard` + sync-wait loop (60×5s, `:149-166`). Skips kubernetes step if no `prod-cluster` Tailnet peer (`:131-139`). Each job uses `concurrency: { cancel-in-progress: false }`.                                                                                                 |
