...
| `cluster-tests.yaml`     | push to main, PR to main                                                                                                                                                    | paths-filter per database package (`packages/{postgres,valkey,clickhouse}/**`) plus a `harness` filter (`scripts/cluster/**`, `scripts/lib/**`, `docker-compose.yaml`, the workflow itself) that runs all three. Single `cluster-tests` job (`timeout-minutes: 45`): setup-node-pnpm + setup-mise (`k3d kubectl helm`) + setup-go, `pnpm cluster k3d deps up && pnpm cluster k3d up`, then per changed package `docker:build`/`dev:push` and `./scripts/cluster/main.sh test <pkg>` sequentially on one shared cluster. `cluster-tests-pass` aggregator for branch protection. |
