...
| `infra/dev.ts`           | Prod-only OVH dev box (`ovh.vps.Vps` `OvhDevVps`, `:3-54`, `protect: true`, 12-month upfront VPS-4 running `Ubuntu 26.04` in `US-WEST-OR`; no cloud-init field on the resource, so guest setup is manual). Credentials come from the `ovh:` provider entry (`sst.config.ts:18-23`) + `OVH_*` env — no explicit `ovh.Provider`. Plus `sst.x.DevCommand` shortcuts for `dev:init`, `dev:destroy`, k3d restart.                                                                                                                                                                                                                                                                                                                                                                                                                                                            |
