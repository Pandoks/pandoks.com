...
| `infra/cluster/cluster.ts` | OVH hybrid k3s topology: four independent Public Cloud/dedicated control-plane/worker pools, vRack network, stable private API DNS with an OVH LB only for multiple control planes, public ingress LBs, node creation, and zero-node Tailnet cleanup. Supporting modules under `infra/cluster/` own network, load balancers, provider resources, and the shared host bootstrap. The Public Cloud project remains required for network and LB resources even when compute is dedicated.                                                                                                                                                                                                                                                                                                                                                                                          |
