...
`import('./infra/sandbox/<name>')` in the `Promise.all` at `:30-44`),
...
`sst.config.ts:30-44` imports every `infra/*.ts` module via literal
`await Promise.all([import('./infra/...')])` (dynamic imports break SST,
see `sst.config.ts:29`).
...
| `infra/github.ts`        | OIDC AWS provider/role (`:72, 77`), prod-only action secrets (OVH pair at `:60-70`), Tailscale CI OAuth (`:128`), Dependabot security-updates off (`:150` — Renovate is the sole security-PR opener; Dependabot alerts stay on as its data source).                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                     |
...
- **SST config**: `sst.config.ts:5` (app name), `:30-44` (infra imports).
