...
| `infra/cluster/cluster.ts` | Config-driven independent OVH K3s clusters for US West, US East, Europe, and Asia. Each enabled region owns its network, API endpoint, mixed Public Cloud/dedicated pools, and ingress LBs; Cloudflare aggregates public origins globally. Supporting modules own topology, regional networking, provider resources, and the shared host bootstrap.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                             |
...
- Topology and catalog selection live in `infra/cluster/config.ts`.
  ...
  Dedicated catalog fields are filled only for an enabled dedicated pool. When
  every regional node count is zero, stale OVH cluster Tailnet entries for the
  stage are reclaimed.
...
- Each OVH account has one stage project/vRack foundation. Each enabled region
  has a separate single-region private network, subnet, gateway, K3s pod/service
  CIDRs, MetalLB range, token, and API DNS name. This keeps managed Gateway/LB
  usage within OVH's supported single-region network topology.
