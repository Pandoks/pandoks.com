...
and wire newly-introduced dependencies into `mise.toml` or the README. Apply
...
### 3b. New-dependency scan (`mise.toml` + README wiring)

A new external tool is a special kind of MISSING-COVERAGE. A versioned
repository tool must have an exact pin in `mise.toml`; a host dependency must
be documented in the README. Detect and wire it in.

**Detect** — a new external tool is in play if the diff (or current code) does any of:

- invokes a binary in shell/CI not already covered by `mise.toml` or the README
...
sed -n '/<summary>Dependencies<\/summary>/,/<\/details>/p' README.md
```
...
1. **Version-shaped tool in the mise registry** (the default) — add an exact
   pin to `mise.toml`'s `[tools]`; `mise install` handles it.
2. **Host prerequisite mise should not manage** (daemons, OS packages, host
   integration) — document the official installation source and purpose in the
   README. Repository scripts must not install, configure, or inventory it.

**Then update the README dependency list.** The root `README.md` has a
`<details><summary>Dependencies</summary>` block — add the new tool there with
a link and a one-line purpose. Also add it to the "Required tools" line in
`.claude/rules/workflows.md` if that rule is in scope.

**Verify** — after wiring a mise tool, run `mise install <tool>` and verify the
resolved command/version. For a host prerequisite, verify its official
installation link and the code path that requires it.
...
ban that) — but declaring a real dependency in mise or the README is in scope
because the dependency already exists in the code.
...
| **NEW-DEPENDENCY**   | A new external tool not covered by `mise.toml` or the README                 | Add a mise pin or document the host prerequisite (§3b)    |
