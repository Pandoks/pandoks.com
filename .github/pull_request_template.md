...
- [ ] _If you have created new templated compute variables_, make sure to add it to the `/scripts/cluster/deploy.sh` script's `cmd_deploy_compute_vars` function.
- [ ] _If you have created a new kubernetes node and control plane_, make sure to add the endpoint to the [`prom-grafana.yaml`](/k3s/overlays) overlays.
