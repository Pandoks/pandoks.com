...
EOF
  ...
  echo "Waiting for Valkey CRD to be established..."
  TIMEOUT=120
  while ! kubectl get crd valkeyclusters.valkey.pandoks.com >/dev/null 2>&1; do
    [ $TIMEOUT -le 0 ] && {
      echo "Timed out waiting for valkeyclusters.valkey.pandoks.com" >&2
      exit 1
    }
    sleep 2
    TIMEOUT=$((TIMEOUT - 2))
  done
  kubectl wait --for=condition=Established crd/valkeyclusters.valkey.pandoks.com --timeout=120s
