#!/bin/sh
# shellcheck shell=sh

set -eu

SCRIPT_DIR="$(CDPATH='' cd -- "$(dirname "$0")" && pwd)"
REPO_ROOT="${SCRIPT_DIR}/../.."
readonly SCRIPT_DIR
readonly REPO_ROOT

. "${REPO_ROOT}/scripts/lib/font.sh"
. "${REPO_ROOT}/scripts/lib/log.sh"

usage() {
  printf "%bUsage:%b pnpm format <command>\n\n" "${BOLD}" "${NORMAL}" >&2
  printf "Format files across the monorepo.\n\n" >&2

  printf "%bWrite mode:%b\n" "${BOLD}" "${NORMAL}" >&2
  printf "  %bjs%b           Prettier over JS/TS/Svelte/MD/YAML/JSON\n" "${GREEN}" "${NORMAL}" >&2
  printf "  %bgo%b           golangci-lint fmt over Go modules\n" "${GREEN}" "${NORMAL}" >&2
  printf "  %bshell%b        shfmt over shell scripts\n" "${GREEN}" "${NORMAL}" >&2
  printf "  %bswift%b        swift-format over Swift sources\n" "${GREEN}" "${NORMAL}" >&2
  printf "  %bkotlin%b       ktlint --format over Kotlin sources\n" "${GREEN}" "${NORMAL}" >&2
  printf "  %ball%b          Format all\n\n" "${GREEN}" "${NORMAL}" >&2

  printf "%bCheck mode (no writes):%b\n" "${BOLD}" "${NORMAL}" >&2
  printf "  %bcheck js%b     Prettier --check\n" "${GREEN}" "${NORMAL}" >&2
  printf "  %bcheck go%b     golangci-lint fmt --diff\n" "${GREEN}" "${NORMAL}" >&2
  printf "  %bcheck shell%b  shfmt -d\n" "${GREEN}" "${NORMAL}" >&2
  printf "  %bcheck swift%b  swift-format lint --strict\n" "${GREEN}" "${NORMAL}" >&2
  printf "  %bcheck kotlin%b ktlint check mode\n" "${GREEN}" "${NORMAL}" >&2
  printf "  %bcheck all%b    Check all\n\n" "${GREEN}" "${NORMAL}" >&2

  exit "${1:-0}"
}

cmd_format_js() {
  cd "${REPO_ROOT}" && prettier --write .
}

cmd_format_go() {
  cd "${REPO_ROOT}" && golangci-lint fmt
}

cmd_format_shell() {
  cd "${REPO_ROOT}"
  git ls-files -z '*.sh' | xargs -0 shfmt -w
}

cmd_format_swift() {
  cd "${REPO_ROOT}"
  git ls-files -z '*.swift' | xargs -0 swift-format format --in-place
}

cmd_format_kotlin() {
  cd "${REPO_ROOT}"
  git ls-files -z '*.kt' '*.kts' | xargs -0 ktlint --format
}

cmd_format_all() {
  cmd_format_js
  cmd_format_go
  cmd_format_shell
  cmd_format_swift
  cmd_format_kotlin
}

cmd_format_check_js() {
  cd "${REPO_ROOT}" && prettier --check .
}

cmd_format_check_go() {
  cd "${REPO_ROOT}" && golangci-lint fmt --diff
}

cmd_format_check_shell() {
  cd "${REPO_ROOT}"
  git ls-files -z '*.sh' | xargs -0 shfmt -d
}

cmd_format_check_swift() {
  cd "${REPO_ROOT}"
  git ls-files -z '*.swift' | xargs -0 swift-format lint --strict
}

cmd_format_check_kotlin() {
  cd "${REPO_ROOT}"
  git ls-files -z '*.kt' '*.kts' | xargs -0 ktlint
}

cmd_format_check_all() {
  cmd_format_check_js
  cmd_format_check_go
  cmd_format_check_shell
  cmd_format_check_swift
  cmd_format_check_kotlin
}

cmd_format_check() {
  [ $# -ge 1 ] || usage 0
  cmd="$1"
  shift

  case "${cmd}" in
    js) cmd_format_check_js ;;
    go) cmd_format_check_go ;;
    shell) cmd_format_check_shell ;;
    swift) cmd_format_check_swift ;;
    kotlin) cmd_format_check_kotlin ;;
    all) cmd_format_check_all ;;
    help | --help | -h) usage ;;
    *)
      log_error "Unknown check language '${cmd}'"
      usage 1
      ;;
  esac
}

main() {
  [ $# -ge 1 ] || usage 0
  cmd="$1"
  shift

  case "${cmd}" in
    js) cmd_format_js ;;
    go) cmd_format_go ;;
    shell) cmd_format_shell ;;
    swift) cmd_format_swift ;;
    kotlin) cmd_format_kotlin ;;
    all) cmd_format_all ;;
    check) cmd_format_check "$@" ;;
    help | --help | -h) usage ;;
    *)
      log_error "Unknown command '${cmd}'"
      usage 1
      ;;
  esac
}

main "$@"
