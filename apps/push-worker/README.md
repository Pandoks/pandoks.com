...
SQS is a Standard queue with long polling, a dead-letter queue, and a five-attempt redrive policy. One worker pod receives up to ten jobs at a time. Successful messages are deleted as a batch; failed messages remain for SQS to retry.
...
Production uses the immutable `tree-<apps/push-worker tree hash>` image tag generated by the existing build workflow. Identical worker contents resolve to the same image across merge strategies; a worker code change rolls the Deployment forward, while an unrelated monorepo commit keeps the previous image.
...
Start with the single pod in `kube/push-worker.yaml`. Each long poll receives up to ten jobs and sends them concurrently, while SQS buffers bursts. Track queue age, DLQ depth, provider throttling, and pod CPU before adding replicas. If queue latency becomes user-visible, add KEDA's SQS scaler with a small maximum replica count; provider throttling should reduce concurrency rather than trigger more pods.
