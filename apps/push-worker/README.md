...
Production uses the immutable `ref-main-<last worker commit>` image tag generated by the existing build workflow. A worker code change therefore rolls the Deployment forward, while an unrelated monorepo commit keeps the last published worker image.
